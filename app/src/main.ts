...
import { EventService } from './services/event.service'; // Import EventService to pass the keys
import * as cors from 'cors'; // Ensure the import statement is added



async function bootstrap() {
  const supabaseUrl = 'https://gzonudkavtxkqpwerxvg.supabase.co';
  const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imd6b251ZGthdnR4a3Fwd2VyeHZnIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcxOTg1Njg2NCwiZXhwIjoyMDM1NDMyODY0fQ.2Bbdh77tSV19d-6mVQXkbPUqnBINC0ESJR5NVAbGYVI';
  ...
  // Pass the Supabase URL and Key to the EventService
  const eventService = app.get(EventService);
  eventService.setSupabaseClient(supabaseUrl, supabaseKey);
